...
class InteractiveUI {
  ...
  /**
   * Display enhanced summary of actions taken with detailed breakdown
   * @param {Object} summary - Enhanced summary statistics
   */
  displaySummary(summary) {
    console.log(`\n${this.colors.blue}${this.colors.bold}═══ Interactive Session Summary ═══${this.colors.reset}`);
    
    // Basic processing stats
    console.log(`\n${this.colors.blue}📊 Processing Overview:${this.colors.reset}`);
    console.log(`   Files processed: ${this.colors.cyan}${summary.filesProcessed}${this.colors.reset}`);
    console.log(`   Console.logs reviewed: ${this.colors.cyan}${summary.totalReviewed}${this.colors.reset}`);
    
    // Actions breakdown
    console.log(`\n${this.colors.blue}⚡ Actions Taken:${this.colors.reset}`);
    console.log(`   ${this.colors.red}🗑️  Deleted: ${summary.deleted}${this.colors.reset}`);
    console.log(`   ${this.colors.green}✅ Kept: ${summary.kept}${this.colors.reset}`);
    console.log(`   ${this.colors.cyan}ℹ️  Converted to info: ${summary.convertedToInfo}${this.colors.reset}`);
    console.log(`   ${this.colors.magenta}❌ Converted to error: ${summary.convertedToError}${this.colors.reset}`);
    
    if (summary.skipped > 0) {
      console.log(`   ${this.colors.yellow}⏭️  Skipped: ${summary.skipped}${this.colors.reset}`);
    }
    
    if (summary.commentedLogsRemoved > 0) {
      console.log(`   ${this.colors.dim}💬 Commented logs removed: ${summary.commentedLogsRemoved}${this.colors.reset}`);
    }
    
    // Security findings
    if (summary.potentiallySensitive > 0) {
      console.log(`\n${this.colors.red}🔒 Security Findings:${this.colors.reset}`);
      console.log(`   ${this.colors.red}⚠️  Potentially sensitive logs: ${summary.potentiallySensitive}${this.colors.reset}`);
      
      if (summary.sensitiveLogsByRisk) {
        if (summary.sensitiveLogsByRisk.high > 0) {
          console.log(`      ${this.colors.red}High risk: ${summary.sensitiveLogsByRisk.high}${this.colors.reset}`);
        }
        if (summary.sensitiveLogsByRisk.medium > 0) {
          console.log(`      ${this.colors.yellow}Medium risk: ${summary.sensitiveLogsByRisk.medium}${this.colors.reset}`);
        }
        if (summary.sensitiveLogsByRisk.low > 0) {
          console.log(`      ${this.colors.blue}Low risk: ${summary.sensitiveLogsByRisk.low}${this.colors.reset}`);
        }
      }
      
      console.log(`   ${this.colors.yellow}💡 Recommendation: Review sensitive logs for security concerns${this.colors.reset}`);
    }
    
    // Smart analysis results
    if (summary.functionalLogsDetected > 0 || summary.catchBlockLogsFound > 0) {
      console.log(`\n${this.colors.blue}🧠 Smart Analysis:${this.colors.reset}`);
      
      if (summary.functionalLogsDetected > 0) {
        console.log(`   ${this.colors.cyan}Functional logs detected: ${summary.functionalLogsDetected}${this.colors.reset}`);
        console.log(`   ${this.colors.green}Functional logs preserved: ${summary.functionalLogsPreserved || 0}${this.colors.reset}`);
      }
      
      if (summary.catchBlockLogsFound > 0) {
        console.log(`   ${this.colors.magenta}Catch block logs found: ${summary.catchBlockLogsFound}${this.colors.reset}`);
        if (summary.catchBlockLogsConverted > 0) {
          console.log(`   ${this.colors.magenta}Catch block logs converted: ${summary.catchBlockLogsConverted}${this.colors.reset}`);
        }
      }
    }
    ...
    if (summary.startTime) {
      const sessionTime = Date.now() - summary.startTime;
      console.log(`\n${this.colors.dim}⏱️  Session time: ${sessionTime}ms${this.colors.reset}`);
    }
    
    // User experience feedback
    const totalActions = summary.deleted + summary.kept + summary.convertedToInfo + summary.convertedToError;
    if (totalActions > 0) {
      console.log(`\n${this.colors.green}✅ Session completed successfully!${this.colors.reset}`);
      
      if (summary.potentiallySensitive > 0) {
        console.log(`${this.colors.yellow}⚠️  Please review flagged sensitive logs before deployment${this.colors.reset}`);
      }
    }
  }
